...
class SegformerEfficientSelfAttention(nn.Module):
    ...
    def __init__(self, config, hidden_size, num_attention_heads, sequence_reduction_ratio):
        ...
        self.logit_scale = nn.Parameter(torch.log(10 * torch.ones((num_attention_heads, 1, 1)))) # swin v2
    ...
    def forward(
        ...
        # (SegFormer) attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))

        # (SegFormer) attention_scores = attention_scores / math.sqrt(self.attention_head_size)

        attention_scores = F.normalize(query_layer, dim=-1) @ F.normalize(key_layer, dim=-1).transpose(-2, -1) # swin v2
